...
def store_model_state(dm, model_to_run):
    ...
    data_to_store = {
         ...
         "mesh_topology": str(dm.problem.mesh.topology.cell_type),
     
     }
    
    try:
        with open(f"{model_to_run}_params.pkl", "wb") as f:
            pickle.dump(data_to_store, f)
            print("Model state saved successfully.")
    except Exception as e:
        print(f"An error occurred while saving the model state: {e}")

def reconstruct_model(dm, model_to_run):
    if upload_model_parameters(model_to_run):
        restore_model()
    
    else:
        ##self.mesh, cell_tags, facet_tags = df.io.gmshio.read_from_msh(self.model_path, MPI.COMM_WORLD, 0)
        dm.experiment.mesh = mesh
        dm.solve()
        
def upload_model_parameters(dm_path):   ##TODO: Make with for loops
    try:
        with open(f"{dm_path}_params.pkl", 'rb') as f:
            model_params = pickle.load(f)
            print(f"Model loaded succesfully")
            model_params["displacement_values"] = np.array(model_params["displacement_values"])
            model_params["mesh_coordinates"] = np.array(model_params["mesh_coordinates"])
...
def restore_model(dm, model_params, mesh):
    displacement_values = model_params["displacement_values"]
    mesh_coordinates = model_params["mesh_coordinates"]
