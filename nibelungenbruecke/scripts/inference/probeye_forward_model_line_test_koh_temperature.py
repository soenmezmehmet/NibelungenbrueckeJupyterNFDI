...
class BridgeModel(ForwardModelBase):
    ...
    def response(self, inp: dict) -> dict:
        ...
        self.f_weight.value = PETSc.ScalarType(
            (
                0,
                -self.forward_model_parameters["model_parameters"]["material_parameters"]["rho"]
                * self.forward_model_parameters["model_parameters"]["material_parameters"]["g"],
                0,
            )
        )
    ...
    def GenerateModel(self):
        ...
        self.f_weight = fem.Constant(
            self.mesh, PETSc.ScalarType((0, -self.material_parameters["rho"] * self.material_parameters["g"], 0))
        )
    ...
    def update_L(self):
        if self.model_parameters["tension_z"] != 0.0:
            self.L = (
                ufl.dot(self.f, self.v) * self.ds_load(1)
                + ufl.dot(self.f_weight, self.v) * ufl.dx
                + ufl.dot(self.T, self.v) * self.ds
            )
        else:
    ...
    def _get_default_parameters():
        default_parameters = {
            ...
            "material_parameters": {"rho": 1.0, "g": 100, "mu": 1, "lambda": 1.25},
            "tension_z": 0.0,
